# Принудительно ставим кодировку UTF8, чтобы баннер был красивым
...
foreach ($key in $urls.Keys) {
    ...
    try {
        # Используем более мощный метод загрузки с UserAgent
        $webClient = New-Object System.Net.WebClient
        $webClient.Headers.Add("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36")
        $webClient.DownloadFile($urls[$key], $destFile)
        ...
        Write-Host "[X] Ошибка скачивания $key! Возможно, блокирует антивирус." -ForegroundColor Red
    }
}

# Проверяем, удалось ли создать пути, чтобы избежать NULL ошибок
...
# 2. Запуск ShellBag Analyzer
if ($exeSba -and (Test-Path $exeSba)) {
    Start-Process $exeSba
}

# 3. Process Hacker и LAV
...
if ($wshell.AppActivate("LastActivityView")) {
    ...
    $wshell.SendKeys("%c") # Учет регистра (Alt+C)
    $wshell.SendKeys("{ENTER}")
}

Write-Host "`n1 - Закрыть`n2 - HolyCheck" -ForegroundColor Green

while($true) {
    $choice = Read-Host "Выбор"
    if ($choice -eq "2") { Start-Process "https://mods.holyworld.me/mods/check" }
    if ($choice -eq "1") {
        Write-Host "[!] Зачистка..." -ForegroundColor Red
