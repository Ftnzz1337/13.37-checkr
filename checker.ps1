...
foreach ($key in $urls.Keys) {
    ...
    $zip = Join-Path $folder "temp.zip"
    
    Write-Host "[>] Downloading $key..." -ForegroundColor Yellow
    Invoke-WebRequest -Uri $urls[$key] -OutFile $zip
...
$exeSbv = Join-Path $paths["SBV"] "ShellBagsView.exe"

# 1. Запуск Everything с запросом
...
Start-Process $exeSbv -WorkingDirectory (Split-Path $exeSbv)
...
while($true) {
    ...
    if ($choice -eq "1") {
        ...
        "Everything", "ProcessHacker", "LastActivityView", "ShellBagsView" | ForEach-Object {
