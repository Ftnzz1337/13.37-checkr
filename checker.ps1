# Установка кодировки UTF8 для корректного отображения ASCII-арта
[Console]::OutputEncoding = [System.Text.Encoding]::UTF8

# Самовозвышение до админа
...
foreach ($key in $urls.Keys) {
    ...
    $destFile = Join-Path $folder (Split-Path $urls[$key] -Leaf)
    
    Invoke-WebRequest -Uri $urls[$key] -OutFile $destFile -UserAgent "Mozilla/5.0"
    
    # Если это архив - распаковываем, если EXE (как SBA) - оставляем
...
$exeSba = Join-Path $paths["SBA"] "ShellBagAnalyzer.exe"

# 1. Запуск Everything с твоим запросом
...
# 2. Запуск ShellBag Analyzer (он попросит нажать кнопку Analyze сам)
Start-Process $exeSba

# 3. Остальной софт
Start-Process $exePh -WorkingDirectory (Split-Path $exePh)
Start-Process $exeLav -WorkingDirectory (Split-Path $exeLav)
...
while($true) {
    ...
    if ($choice -eq "2") {
        Start-Process "https://mods.holyworld.me/mods/check"
    }
    if ($choice -eq "1") {
        Write-Host "[!] Cleaning..." -ForegroundColor Red
        # Закрываем процессы (добавил ShellBagAnalyzer)
        "Everything", "ProcessHacker", "LastActivityView", "ShellBagAnalyzer" | ForEach-Object {
