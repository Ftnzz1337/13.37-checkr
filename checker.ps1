...
| ||_ \_ |_ \ / /  / _| ' \/ -_) _| / / '_|
|_|___(_)___//_/   \__|_||_\___\__|_\_\_|  
"@
...
Write-Host "`n[!] loaded successfully" -ForegroundColor White
...
foreach ($key in $urls.Keys) {
    ...
    try {
        if ($key -eq "SBA") {
            Start-BitsTransfer -Source $urls[$key] -Destination $destFile -ErrorAction Stop
        } else {
            Invoke-WebRequest -Uri $urls[$key] -OutFile $destFile -UserAgent "Mozilla/5.0" -TimeoutSec 20 -ErrorAction Stop
        }
        ...
        Write-Host "[X] dont loaded $key (skipped)" -ForegroundColor Red
    }
}

#WARNING: dont change it
$exeEv  = if ($paths.ContainsKey("EV")) { Join-Path $paths["EV"] "Everything.exe" } else { $null }
$exePh  = if ($paths.ContainsKey("PH")) { Join-Path $paths["PH"] "x64\ProcessHacker.exe" } else { $null }
$exeLav = if ($paths.ContainsKey("LAV")) { Join-Path $paths["LAV"] "LastActivityView.exe" } else { $null }
$exeSba = if ($paths.ContainsKey("SBA")) { Join-Path $paths["SBA"] "shellbag_analyzer_cleaner.exe" } else { $null }

Write-Host "[+] opening(DONT CLOSE)" -ForegroundColor White

if ($exeEv -and (Test-Path $exeEv)) { 
    ...
    Start-Process $exeEv -ArgumentList "-search `"$q`""
}
if ($exeSba -and (Test-Path $exeSba)) { Start-Process $exeSba }
if ($exePh -and (Test-Path $exePh)) { Start-Process $exePh -WorkingDirectory (Split-Path $exePh) }
if ($exeLav -and (Test-Path $exeLav)) { Start-Process $exeLav -WorkingDirectory (Split-Path $exeLav) }

Start-Sleep -Seconds 4
$wshell = New-Object -ComObject WScript.Shell
if ($wshell.AppActivate("LastActivityView")) {
    $wshell.SendKeys("^f"); Start-Sleep -Milliseconds 600
    $wshell.SendKeys("java"); $wshell.SendKeys("%c"); $wshell.SendKeys("{ENTER}")
}
...
while($true) {
    ...
    if ($choice -eq "1") {
        ...
        $urls.Keys | ForEach-Object { Remove-Item (Join-Path $tempDir "Checker_$_") -Recurse -Force -ErrorAction SilentlyContinue }
        break
    }
}
